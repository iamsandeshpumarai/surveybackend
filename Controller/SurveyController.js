...
const createSurvey = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'Survey created successfully', id: surveyData._id });
  } catch (err) {
...
const getSurveyData = async (req, res) => {
    try {
        const userdata = await UserSurvey.findOne({userId:req.id})
...
const getuserSurveyData = async (req, res) => {
  ...
  console.log("iuam on surveydata")
    try {
        const userdata = await UserSurvey.findOne({userId:req.params.id})
...
module.exports = { createSurvey, getSurveyData, updateSurvey, deleteSurveyData,getuserSurveyData };